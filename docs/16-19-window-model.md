# 16-19 window: two activities inside one window
...
- Journey configs key off `{WhatToChange}_{CheckingWindowType}.json`
  (`IQuestionFlowService.GetConfigAsync`), so every journey belongs to a change type, not to an
  activity.
...
2. The card still lands the user on **Check your pupil data**. That page offers the two activities.
3. The model is an **activity child collection** on `CheckingWindow`.

---

## The model

`CheckingWindow` keeps its outer `StartDate`/`EndDate`. It gains an `Activities` child collection.
The existing `Datasets` collection **moves down onto the activity**: a dataset belongs to the
activity that consumes it, not to the window.

```
CheckingWindow
  Id, KeyStage, CheckingWindowType, Title
  StartDate 07 Oct    EndDate 31 Mar      <- outer, the union of all activities
  Activities [
...
- KS4 June and KS2 get **one** activity row, `PupilData`, with the same dates as the window. Their
...
- The outer pair is the **only** thing that decides whether the window shows a card. It must equal
  the union of the activity dates.
- A window that is open but has **no** activity open still shows its card. The user goes into a
  read-only page: pupil list, search, CSV and ZIP downloads, and no action options at all. Activity
  state controls actions, never visibility.

Why this shape and not the alternatives:

- A second date pair on `CheckingWindow` (`PupilDataEndDate`) hardcodes "exactly two phases, one of
  which is pupil data". A third activity later needs another migration.
...
- The `Datasets` collection already set the precedent for child rows on `CheckingWindow`. This
  follows it, and then reparents `Datasets` onto the activity — see below.

### New types

```csharp
// Domain/Enums/CheckingWindowActivityType.cs
public enum CheckingWindowActivityType
{
...
```csharp
// Persistence/Entities/CheckingWindowActivity.cs
public sealed class CheckingWindowActivity
{
    public Guid Id { get; init; }
    public Guid CheckingWindowId { get; set; }
    public CheckingWindowActivityType ActivityType { get; init; }
...
### Datasets belong to the activity

`CheckingWindowDataset.CheckingWindowId` becomes `CheckingWindowActivityId`. `CheckingWindow` no
longer holds `Datasets` directly; it reaches them through `Activities`.

This is the right home for them. A dataset is an input to one activity. The two 16-19 pupil CSVs
feed pupil data checking. Results enquiry will have its own inputs, on its own dates, validated
against its own schemas. Hanging both off the window would put unrelated files in one flat list with
nothing to say which activity each serves.

Three consequences fall out of the move:

1. **Ingress runs per activity, not per window.** `16-19-pupils-plan.md` requires the two 16-19
   pupil CSVs to be ingested in a **single run**, because a second run would wipe the first run's
   output. That constraint now applies **within an activity**. Two activities are two independent
   runs, which is what you want — results-enquiry data must not have to be re-uploaded to correct a
   pupil file.

2. **The blob layout must become activity-scoped.** `CsvSchemaFileProcessor` writes
   `data/{schoolId}_pupils.json` (`:300`) and its clear sweep deletes the whole `data/` prefix
   (`:411`). If two activities ingest into the same `{windowId}` container, the second run destroys
   the first activity's output. Give each activity its own prefix — `{windowId}` container,
   `{activity}/data/{laestab}.json` — and scope the sweep to that prefix. Pupil data keeps its
   current path shape under the new prefix.

3. **`HasPupilData` becomes activity-scoped.** The landing page existence check
   (`IPupilDataBlobClient.HasPupilDataAsync(windowId, laestab)`) must ask about the pupil-data
   activity's prefix. Note this is now only about whether the read-only pupil content can render —
   it no longer decides whether a card appears.

Also per-activity: the window admin wizard's schema and ingress steps, and `WindowValidated` — a
window is not validated as a whole any more, each activity is.
...
1. Create `CheckingWindowActivities`. Backfill one `PupilData` row per existing window, copying that
   window's `StartDate` and `EndDate`.
2. Add `CheckingWindowActivityId` to `CheckingWindowDatasets` and point every existing row at its
...
## Where "is this activity open" is answered

One rule, in one place, in Application. Nothing else may compare dates.

```csharp
// Application/WindowManagement/IWindowActivityService.cs
bool IsOpen(CheckingWindowDto window, CheckingWindowActivityType activity, DateTime now);
IReadOnlyList<CheckingWindowActivityType> OpenActivities(CheckingWindowDto window, DateTime now);
DateTime? EndDateFor(CheckingWindowDto window, CheckingWindowActivityType activity);
```

Fail closed: a window with **no** activity row for a type is closed for that type. A window whose
activity list is empty is closed for everything. That way a half-configured window cannot open a
journey by accident.

Fail closed applies to **actions only**. `OpenActivities` returning an empty list must never remove
the card or the pupil data. The read-only content is always available for the whole outer window.
...
The window DTO that reaches the Web layer must carry the activity list, so the repository projection
in `LandingPageRepository` and the `CheckYourPupilData` window read both need the extra `.Select`.
...
not any activity is open. The card title is the window title ("16-19 Data").

Consider showing the pupil-data deadline on the card as a hint while that activity is open. Confirm
...
2. The controller builds the visible option list from `OpenActivities(...)`. `RequestChange` and
   `Confirm` both belong to `PupilData` and both disappear together when it closes.
   `ResultsEnquiry` appears only while that activity is open.
...
- `CheckYourPupilDataController` next-steps POST — reject a closed activity.
...
- `JourneyController` — its existing `IsSessionReady` guard runs on every action
  ...
  also require the journey's activity to be open. One change covers every journey action.
...
## How a journey knows its activity
...
Results enquiry journeys are a different **activity**, not a different change type. Two options,
both compatible with the model above. Pick when the results-enquiry flows are specified:

**A — new `WhatToChange` members, with an activity attribute.** Add e.g.
`WhatToChange.ResultsEnquiry`. Map each member to its activity in one lookup in Application. The
config naming rule is untouched. Smallest change; works while results enquiry is one journey.

**B — activity becomes part of the config key.** The blob becomes
`{Activity}_{WhatToChange}_{CheckingWindowType}.json`, with the existing files treated as
`PupilData_*`. Cleaner if results enquiry grows several distinct journeys.

Either way `RequestState` should carry the activity so `IsSessionReady` can gate on it, and so
`ChangeRequest` rows and the Amendment Requests grid can tell the two populations apart.
...
1. **Drafts across the boundary.** A user saves a pupil-data draft on 17 Oct and returns on 19 Oct.
   ...
   activity. Options: block resume with a clear message, or allow resume but block submit. Product
   decision. The gate must be in `IsSessionReady` either way.
2. **Are results enquiries pupil-centric?** If a results enquiry starts by picking a pupil, it can
   ...
   whole feature.
3. **Amendment Requests grid.** Does it show both activities' requests in one list, or split them?
4. **Window admin wizard.** The activity dates need a step. Suggest: for a window type with more
   than one activity, the wizard asks for each activity's dates and derives the outer pair as their
   union, so the two can never disagree.
...
Write the note above to `16-19-window-model.md` at the repo root, next to `16-19-pupils-plan.md`
and `16-19-reuse-investigation.md`, which follow the same convention.

Reconcile `16-19-pupils-plan.md`. Two of its statements are superseded by this note:

- Its step 2 puts the dataset collection on `CheckingWindow`. Datasets now hang off the activity.
- Its step 1 treats "one ingress run per window" as the unit. The unit is now the activity, and
  `clearExistingFiles` must be scoped to the activity's blob prefix rather than all of `data/`.

Its "Deliberately out of scope" section also defers 16-19 journey configs. This note is where the
activity model for those now lives.
