...
- **Edit-mode only on the UI.** The seed button is rendered inside the edit-mode disclosure, behind a `@if (ViewBag.IsEditMode)` guard. The endpoint itself has `[ValidateAntiForgeryToken]` but is not yet `[Authorize]`-gated — that lands with the rest of the auth wiring. Don't deploy with this endpoint reachable in prod until that's done.
