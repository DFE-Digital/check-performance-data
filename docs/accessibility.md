...
**A select enhanced with `enhanceSelectElement` must pass `defaultValue: ''`** (AB#301933). Left unset, the library copies the *selected* option's text into the new input as its value; nothing is preselected on our pickers, so that is the placeholder row ("Select revised grade" / "Select") — the field opened already "filled in", a screen reader announced a value, and `showAllValues` filtered the options against that text and found nothing until the user cleared it. An empty `defaultValue` leaves the field empty; the library still overrides it with a genuinely selected option's text (validation redisplay, Back, Change), so restoration needs no code. Do not reach for the component's `placeholder` option instead: GOV.UK guidance is not to use placeholder text, and it is the same defect by another route.

`Views/Journey/_GradeSelect.cshtml` and `Views/Journey/_SyllabusSelect.cshtml` do this, pinned by `AccessibilityAuditViewTests`. `ResultSearch.cshtml` is unaffected because its placeholder option has empty text, which `ResultSearchViewSourceTests.Nothing_is_preselected` pins; give that option text and the rule applies to it too.

This depends on the library overriding `defaultValue` whenever the select holds a value (`configurationOptions.selectElement.value || configurationOptions.defaultValue === undefined` in `enhanceSelectElement`; upstream `alphagov/accessible-autocomplete` `main` still carried it when checked on 9 September 2026). If `accessible-autocomplete` is ever upgraded, run the E2E suite — `IncorrectGradeEnquiryTests.AChosenGradeIsStillRestoredWhenTheUserComesBackToThePage` and `MissingQualificationEnquiryTests.Answered_pickers_are_restored_on_a_validation_redisplay` are the only tests that check restoration, and E2E is not on the always-on CI gate.

**Passing `onConfirm` to `enhanceSelectElement` replaces the library's default, and that default is what marks the matching `<option>` selected.** The hidden select is what the form posts, so an override must do that first — compare `option.textContent || option.innerText` to the confirmed label untrimmed, exactly as the library does — and only then do its own work. `Views/Journey/ResultSearch.cshtml` does this to reveal the chosen result's details (AB#301934), pinned by `ResultSearchViewSourceTests.Confirming_a_suggestion_reveals_its_details_and_still_selects_the_option_that_posts`. The revealed details sit in a polite live region (`aria-live="polite"`) so a user who confirms with Enter is told what appeared below the field; keep it polite — the library already announces the selection itself.

**A `Locator.PressAsync` call refocuses its element before dispatching the key.** Refocusing an already-open enhanced input mid-interaction (e.g. between an ArrowDown and a following Enter) can trigger the library's `handleComponentBlur`, which closes the menu and clears the highlighted option — so a second key sent that way can silently land on nothing. `IncorrectGradeEnquiryTests.ChoosingAResultWithTheKeyboardShowsItsDetails` drives such a sequence via `Page.Keyboard` instead, which sends keys to whatever already has focus with no refocus step.
