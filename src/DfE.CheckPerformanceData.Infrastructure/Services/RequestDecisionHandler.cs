...
public class RequestDecisionHandler : IRequestDecisionHandler
{
    ...
    private async Task HandleApprovedAsync(ApprovedRequestMessage? message, CancellationToken token)
    {
        ...
        _logger.LogInformation(
            ...
            message.WindowId, message.RequestId, message.DecisionType);
        ...
        var ticketRequest = new CreateTicketRequestDto
        {
            Ticket = new CreateTicketDto
            {
                ...
                Description = $"Request {message.RequestId} for window {message.WindowId} has been approved.\nReason: {message.Reason}",
                Status = "open",
                Priority = "normal",
                Type = "task"
            }
    ...
    private async Task HandleRejectedAsync(RejectedRequestMessage? message, CancellationToken token)
    {
        ...
        var ticketRequest = new CreateTicketRequestDto
        {
            Ticket = new CreateTicketDto
            {
                ...
                Description = $"Request {message.RequestId} for window {message.WindowId} has been rejected.\nReason: {message.Reason}",
                Status = "open",
                Priority = "normal",
                Type = "task"
            }
    ...
    private async Task HandleScrutinyAsync(ScrutinyMessage? message, CancellationToken token)
    {
        ...
        _logger.LogInformation(
            ...
            message.WindowId, message.RequestId);
        ...
        var ticketRequest = new CreateTicketRequestDto
        {
            Ticket = new CreateTicketDto
            {
                ...
                Description = $"Request {message.RequestId} for window {message.WindowId} requires scrutiny.\nReason: {message.Reason}",
                Status = "open",
                Priority = "high",
                Type = "task"
            }
