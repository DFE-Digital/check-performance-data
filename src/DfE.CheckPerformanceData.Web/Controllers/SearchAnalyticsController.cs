...
// 4 stat tiles + top-10 tables + empty-state panel. Downstream plans hang extra actions
// (volume chart drill-in, top-pages drill-in, session drill-in) off this same controller
// under the shared /admin/Search route.
...
public sealed class SearchAnalyticsController : Controller
{
    ...
    public SearchAnalyticsController(ISearchAnalyticsQueryService query)
    {
        _query = query;
    }
        ...
        CancellationToken ct = default)
    {
        ...
        if (hasData)
        {
            ...
            // 20 rows the view renders a single inset panel and never reads the summary or
            // the top-N tables.
        ...
        // Explicit path because the view lives under Views/Admin/Search/, which is not the
        // default MVC convention for a controller named SearchAnalyticsController (that would
        // resolve to Views/SearchAnalytics/). The admin grouping is deliberate — every
        // search-admin surface lives under /admin/Search/, and the file tree mirrors that.
