...
// The encoded UAT inventory: every human-checkable item from 03.10-UAT.md + 03.13-UAT.md as a
// typed list (the guided runner), plus the ids of the automated-only items the coverage panel
// resolves against uat-coverage.json / uat-status.json. Encoding the inventory in code keeps the
...
{
    // Drive-traffic and failure endpoints reuse the existing dev controllers; the UAT POST drivers
    // post to the /dev/uat/* actions which delegate to those same paths and redirect back.
    private const string DriveApproved = "/dev/uat/drive?outcome=approved";
    private const string DriveRejected = "/dev/uat/drive?outcome=rejected";
    private const string DriveScrutiny = "/dev/uat/drive?outcome=scrutiny";
    private const string InjectFailure = "/dev/uat/inject-failure";
    private const string SeedDlq = "/dev/uat/seed-dlq";
    ...
    public static readonly IReadOnlyList<UatItem> Interactive = new List<UatItem>
    {
        new("3.13-1", UatPhase.Phase313, "Debug menu present and gated",
            "The Debug nav entry is visible here; with Dev:ToolsEnabled off the /dev/* surfaces 404.",
            new[] { Link("Open dashboard", Dashboard) }),

        new("3.13-2", UatPhase.Phase313, "Zendesk-styled ticket preview",
            "The outbox renders a faithful Zendesk-styled ticket, not raw JSON.",
            new[] { Link("Open outbox", Outbox) }, Outbox),

        new("3.13-3", UatPhase.Phase313, "Failure-and-recovery demo",
        ...
        new("3.13-4", UatPhase.Phase313, "Queues page redesign",
            "Three queue panels with top-5 messages and a view-all link.",
            new[] { Link("Open queues", Queues) }, Queues),

        new("3.13-5", UatPhase.Phase313, "Live animated board",
        ...
        new("3.13-6", UatPhase.Phase313, "Stats dashboard",
    ...
    private static UatAction Link(string label, string url) => new(label, url, UatActionMethod.Link);
    private static UatAction Post(string label, string url) => new(label, url, UatActionMethod.Post);
}
