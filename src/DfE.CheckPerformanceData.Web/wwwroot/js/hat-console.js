// HAT test console client behaviour. Pure progressive enhancement: the runner, its verdict radios
...
// and enriches the automated-coverage panel from the served hat-coverage.json / hat-status.json.
// No server-side storage of results — this is a dev aid only.
(function () {
  'use strict';

  var STORAGE_KEY = 'cpd-hat-results-v1';
  ...
  function itemKey(el) {
    var item = el.closest('[data-hat-item]');
    if (!item) return null;
    return item.getAttribute('data-hat-phase') + ':' + item.getAttribute('data-hat-item');
  }

  // --- Restore persisted verdicts + notes -----------------------------------------------------

  function restore(results) {
    document.querySelectorAll('[data-hat-item]').forEach(function (item) {
      var key = item.getAttribute('data-hat-phase') + ':' + item.getAttribute('data-hat-item');
      ...
      if (saved.notes) {
        var notes = item.querySelector('[data-hat-notes]');
  ...
  function updateProgress() {
    var total = document.querySelectorAll('[data-hat-item]').length;
    var done = 0;
    document.querySelectorAll('[data-hat-item]').forEach(function (item) {
      if (item.querySelector('input[type="radio"]:checked')) done += 1;
    });
    var text = document.querySelector('[data-hat-progress-text]');
  ...
  function applyFilter(value) {
    document.querySelectorAll('[data-hat-item]').forEach(function (item) {
      var phase = item.getAttribute('data-hat-phase');
  ...
  function syncBatch() {
    var batch = document.querySelector('[data-hat-batch]');
    if (!batch) return;
    var n = parseInt(batch.value, 10);
    if (isNaN(n) || n < 1) n = 1;
    if (n > 20) n = 20;
    document.querySelectorAll('[data-hat-batch-mirror]').forEach(function (input) {
      input.value = n;
    });
  }
  ...
  function clearResults() {
    if (!window.confirm('Clear all HAT results stored in this browser?')) return;
    try {
  ...
  function exportResults() {
    ...
    document.querySelectorAll('[data-hat-item]').forEach(function (item) {
      ...
      out.items.push({
        id: item.getAttribute('data-hat-item'),
        phase: item.getAttribute('data-hat-phase'),
    ...
    a.href = url;
    a.download = 'hat-results-' + new Date().toISOString().slice(0, 10) + '.json';
  ...
  function enrichCoverage() {
    var table = document.querySelector('[data-hat-coverage]');
    if (!table) return;
    ...
    fetch(coverageUrl).then(function (r) { return r.ok ? r.json() : null; }).then(function (manifest) {
      ...
      document.querySelectorAll('[data-hat-coverage-id]').forEach(function (row) {
        var entry = byId[row.getAttribute('data-hat-coverage-id')];
        if (!entry) return;
      ...
      // Second pass: overlay live pass/fail/last-run from hat-status.json when present.
      return fetch(statusUrl).then(function (r) { return r.ok ? r.json() : null; }).then(function (status) {
        ...
        document.querySelectorAll('[data-hat-coverage-id]').forEach(function (row) {
          var s = statusById[row.getAttribute('data-hat-coverage-id')];
  ...
  function copyCommand(button) {
    var row = button.closest('[data-hat-coverage-id]');
    if (!row) return;
    var command = row.querySelector('[data-coverage-command]');
  ...
  function init() {
    ...
    document.addEventListener('change', function (e) {
      if (e.target.matches('[data-hat-verdict] input[type="radio"]')) {
        recordVerdict(results, e.target);
        updateProgress();
      } else if (e.target.matches('[data-hat-filter] input[type="radio"]')) {
        applyFilter(e.target.value);
      } else if (e.target.matches('[data-hat-batch]')) {
        syncBatch();
      }
    });

    document.addEventListener('input', function (e) {
      if (e.target.matches('[data-hat-notes]')) recordNotes(results, e.target);
    });

    document.addEventListener('click', function (e) {
      if (e.target.matches('[data-hat-clear]')) clearResults();
      else if (e.target.matches('[data-hat-export]')) exportResults();
