...
public class IncludeHandoffConflictTests
{
    ...
    private static readonly Guid IncludedNamesakeId = Guid.Parse("eeeeeeee-eeee-eeee-eeee-eeeeeeeeeeee");
    ...
    private static readonly PupilDuplicateCheckResult MultipleSameNameCheck =
        PupilDuplicateCheckResult.Build([SingleMatch, new DuplicateMatch
        {
            Id = IncludedNamesakeId,
            Firstname = "John",
            Surname = "Doe",
            DateOfBirth = "02/02/2010",
            Identifier = "UPN004",
    ...
    [Fact]
    public async Task SwitchToInclude_WhenIncludedNamesakeAlreadyOnScreen_ProceedsToSeed()
    {
        // The Multiple list already showed the included John Doe row (status column "Included"), so
        // switching to include the non-included John Doe is an informed choice — the guard must not
        // re-block an included record the duplicate-check page already surfaced (E2E T034).
        SetupSession(AddSessionAwaitingInclude(MultipleSameNameCheck));
        _flowService.GetConfigAsync(WhatToChange.Include, CheckingWindowType.KS4June).Returns(IncludeConfig);
        _pupilDataService.GetPupilAsync(WindowId, MatchPupilId).Returns(MatchPupil);
        _pupilDataService.GetPupilSuggestionsAsync(WindowId, "John Doe", PupilFilter.Included)
            .Returns([new PupilSuggestionDto(IncludedNamesakeId, "Doe, John", "John", "Doe", "02/02/2010")]);
