...
// AB#296648/AB#297848/AB#298704: the close sweep must skip results enquiries — every kind.
//
// This path rebuilds a PUPIL AMENDMENT ticket from a journey blob. An enquiry's QAN, session,
// current and revised grade have no place in that shape, so replaying one would create a malformed
// ticket — and worse, would flip the row from SubmittedUnCommitted to SubmittedCommitted, so the
// real enquiry-to-Zendesk dispatch (a separate story) could never find it again.
//
// Ported from AdminRequestsServiceEnquiryGuardTests. Under the per-exercise sweep this guard is now
// what makes the Results Enquiry close button enqueue NOTHING while still cancelling that
// exercise's drafts — the intended behaviour, not an accident of the row filter.
//
...
public sealed class CloseExerciseServiceEnquiryGuardTests
{
    ...
    public CloseExerciseServiceEnquiryGuardTests()
    {
    ...
    [Fact]
    public async Task Closing_the_results_enquiry_exercise_still_cancels_its_drafts()
    {
    ...
    [Theory]
    [MemberData(nameof(EnquiryKinds))]
    public async Task An_enquiry_journey_stamped_onto_the_pupil_data_exercise_is_still_skipped(WhatToChange enquiry)
    {
